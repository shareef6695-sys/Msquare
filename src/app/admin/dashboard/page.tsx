...
export default function AdminDashboardPage() {
  ...
  const summary = useMemo(() => {
    ...
    const lcUnderReview = lcOrders.length;
    ...
    <AdminLayout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label: "Pending merchants", value: String(summary.pendingMerchants), icon: <Users className="w-5 h-5" />, href: "/admin/merchants" },
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {[
          ...
          { label: "Rejected merchants", value: String(summary.rejectedMerchants), icon: <Gavel className="w-5 h-5" />, href: "/admin/merchants" },
          { label: "Escrow under review", value: String(summary.escrowUnderReview), icon: <ShieldCheck className="w-5 h-5" />, href: "/admin/payments" },
          { label: "LC under review", value: String(summary.lcRequestsUnderReview), icon: <FileText className="w-5 h-5" />, href: "/admin/lc-requests" },
        ].map((stat) => (
          <Link key={stat.label} href={stat.href}>
            <Card className="hover:shadow-sm transition-all">
              <CardContent className="p-6 flex items-center gap-4">
        ...
        <Card>
          ...
          <CardContent className="p-6">
            <div className="rounded-2xl border border-gray-200/60 bg-gray-50 px-4 py-3">
              ...
              <div className="text-2xl font-black text-gray-900 mt-1">{summary.pendingDisputes}</div>
            </div>
